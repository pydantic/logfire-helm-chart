...
| podSecurityContext | object | `{"fsGroup":1000,"fsGroupChangePolicy":"OnRootMismatch"}` | Pod SecurityContext (https://kubernetes.io/docs/tasks/configure-pod-container/security-context/#set-the-security-context-for-a-pod) See: https://kubernetes.io/docs/reference/kubernetes-api/workload-resources/pod-v1/#security-context for details The Fusionfire and bcache images run as uid/gid 1000. Chart-managed volumes (scratch and ingest data) mount root-owned without an fsGroup, so the default group-owns them once on first mount; `fsGroupChangePolicy: OnRootMismatch` skips the recursive chown after that. A user-supplied `fsGroup` still wins. |
